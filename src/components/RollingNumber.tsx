...
export default function RollingNumber({ value, showSymbol = true, className = "" }: RollingNumberProps) {
  ...
  useEffect(() => {
    ...
    return () => unsubscribe();
  }, [spring, activeCurrency, showSymbol]);

  // Initial render safety
  useEffect(() => {
    setDisplayString(format(value, showSymbol));
  }, [value, activeCurrency, format, showSymbol]);
